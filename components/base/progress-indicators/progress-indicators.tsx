...
export const ProgressBarBase = ({ value, min = 0, max = 100, className, progressClassName }: ProgressBarProps) => {
    ...
    return (
        <div
            ...
            aria-valuemin={min}
            aria-valuemax={max}
            className={cx("h-2 w-full overflow-hidden rounded-md bg-secondary/20", className)}
        >
            <div
                // Use transform instead of width to avoid layout thrashing (and for smoother animation)
                style={{ transform: `translateX(-${100 - percentage}%)` }}
                className={cx("size-full rounded-md bg-primary transition duration-75 ease-linear", progressClassName)}
            />
        </div>
    );
};
        ...
        case "right":
            return (
                <div className="flex items-center gap-3">
                    {baseProgressBar}
                    <span className="shrink-0 text-sm font-medium text-muted-foreground tabular-nums">{formattedValue}</span>
                </div>
            );
        case "bottom":
            return (
                <div className="flex flex-col items-end gap-2">
                    {baseProgressBar}
                    <span className="text-sm font-medium text-muted-foreground tabular-nums">{formattedValue}</span>
                </div>
            );
        case "top-floating":
            return (
                <div className="relative flex flex-col items-end gap-2">
                    ...
                    <div
                        ...
                        className="absolute -top-2 -translate-x-1/2 -translate-y-full rounded-lg bg-popover px-3 py-2 shadow-lg ring-1 ring-border"
                    >
                        <div className="text-xs font-semibold text-popover-foreground tabular-nums">{formattedValue}</div>
                    </div>
                </div>
            );
        case "bottom-floating":
            return (
                <div className="relative flex flex-col items-end gap-2">
                    ...
                    <div
                        style={{ left: `${percentage}%` }}
                        className="absolute -bottom-2 -translate-x-1/2 translate-y-full rounded-lg bg-popover px-3 py-2 shadow-lg ring-1 ring-border"
                    >
                        <div className="text-xs font-semibold text-popover-foreground">{formattedValue}</div>
                    </div>
                </div>
            );
        default:
