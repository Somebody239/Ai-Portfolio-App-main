...
export function PortfolioAdvisor() {
    ...
    return (
        <div className="space-y-6">
            <form onSubmit={handleSubmit} className="bg-zinc-900/40 rounded-xl border border-zinc-800 p-6 space-y-4">
                <div>
                    <label className="block text-sm font-medium text-zinc-400 mb-2">Your Interests (comma-separated) *</label>
                    <input
                        type="text"
                        required
                ...
                <div>
                    <label className="block text-sm font-medium text-zinc-400 mb-2">Career Goals *</label>
                    <textarea
                        required
                ...
                <div>
                    <label className="block text-sm font-medium text-zinc-400 mb-2">Current Activities (comma-separated)</label>
                    <textarea
                ...
                <button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-zinc-700 text-white px-6 py-3 rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
                >
                    {loading ? (
                        <>
            ...
            {result && (
                <div className="space-y-6">
                    ...
                    <div className="bg-zinc-900/40 rounded-xl border border-zinc-800 p-6">
                        ...
                        <div className="grid gap-4">
                            {result.competitions.map((comp, i) => (
                                <div key={i} className="border border-zinc-800 rounded-lg p-4 bg-zinc-900/20">
                                    <div className="flex items-start justify-between mb-2">
                                        <h4 className="font-semibold text-white">{comp.title}</h4>
                                        {comp.difficulty && (
                                            <span className={`text-xs px-2 py-1 rounded ${comp.difficulty === 'Beginner' ? 'bg-emerald-950/50 text-emerald-400 border border-emerald-800/50' :
                                                comp.difficulty === 'Intermediate' ? 'bg-amber-950/50 text-amber-400 border border-amber-800/50' :
                                    ...
                                    {comp.url && (
                                        <a href={comp.url} target="_blank" rel="noopener noreferrer" className="text-sm text-indigo-400 hover:text-indigo-300 hover:underline flex items-center gap-1">
                    ...
                    <div className="bg-zinc-900/40 rounded-xl border border-zinc-800 p-6">
                        <h3 className="text-lg font-semibold text-white mb-4">Volunteer Opportunities</h3>
                        <div className="grid gap-4">
                            {result.volunteerOpportunities.map((opp, i) => (
                                <div key={i} className="border border-zinc-800 rounded-lg p-4 bg-zinc-900/20">
                                    <div className="flex items-start justify-between mb-2">
                                        ...
                                        {opp.type && (
                                            <span className="text-xs px-2 py-1 rounded bg-blue-950/50 text-blue-400 border border-blue-800/50">
                                                {opp.type}
                                            </span>
                                        )}
                                    </div>
